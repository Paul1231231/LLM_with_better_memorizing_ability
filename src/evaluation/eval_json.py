...
code_cohesion_metric = ConversationalGEval(
    name="Code Cohesion",
    criteria="""
    Evaluate whether the assistant preserves and improves the user's original code appropriately.

    Scoring guide (0.0 to 1.0):
    - 1.0: Minimal, targeted edits; preserves original structure, logic flow, and naming; all changes directly address the user request.
    - 0.8: Mostly preserved; small unnecessary refactors but still clearly grounded in original code and request.
    - 0.6: Partial preservation; noticeable rewrites or renaming that were not strictly necessary.
    - 0.4: Major rewrite with limited need; weak alignment with requested fix/improvement.
    - 0.0-0.2: Complete replacement or off-task response; ignores original code constraints.

    Rules:
    1) Debug task: Prefer smallest possible fix. Penalize broad rewrites.
    2) Improvement/refactor task: Improvements allowed, but must retain core behavior and traceability to original code.
    3) Preserve variable/function names unless change is required for correctness, clarity, or API compatibility.
    4) Any introduced logic must be necessary and relevant to the user request.
    5) Penalize changes that alter expected behavior without justification.
    """,
    model=model,
    strict_mode=False,
    threshold=0.5
